...
int mem_is_free(void *ptr)
{
  ...
  while (curseur_blocks_libres != NULL)
  {
    ...
    if (((ptr > (curseur_blocks_libres + sizeof(mem_free_block_t))) &&
         (ptr < (curseur_blocks_libres +
                 ((mem_free_block_t *)curseur_blocks_libres)->taille_total))) ||
        (ptr == curseur_blocks_libres))
    {
      return 1;
    }
...
int mem_is_a_free_mem_header(void *zone)
{
  mem_free_block_t *curseur_blocks_libres = (void *)gbl_alloc->first_free_block;

  // Parcours de toute la mémoire
  while (curseur_blocks_libres != NULL)
  {
    if (zone == curseur_blocks_libres)
    {
      return 1;
    }
...
void *mem_alloc(size_t size)
{
  ...
  if (total_size_a_allouer > mem_space_get_size() -
                                 (size_t)sizeof(allocator_t) -
                                 (size_t)sizeof(mem_busy_block_t))
  {
    return NULL;
  }
  ...
  if (free_block != NULL)
  {
    ...
    if (free_block->taille_total > total_size_a_allouer)
    {
      ...
      new_free_block->taille_total =
          update_size - total_size_a_allouer;
    ...
    else
    {
      ...
      if (free_block == gbl_alloc->first_free_block)
      {
        gbl_alloc->first_free_block = (mem_free_block_t *)free_block->ptr_next_free;
      }
      else
      {
...
{
  // !!!!!!!!!!!!!!! PENSER A AJOUTER SIZEOF(mem_busy_block_t) QUAND ON VEUX FREE DANS LE MEM_SHELL EN UTILISANT "l" !!!!!!!!!!!!!!!!
  if (zone == NULL || mem_is_a_free_mem_header(zone - sizeof(mem_busy_block_t)) == 1)
  {
    // Ne rien faire, la zone est déjà un bloc libre
    printf("Mémoire déjà libre / la zone fournie doit être un busy header.\n");
    return;
  }
...
void mem_show(void (*print)(void *, size_t, int free, void *))
{
  ...
  {
    if (mem_is_a_free_mem_header(curseur))
    {
      mem_free_block_t *current_free = ((mem_free_block_t *)curseur);
      print(current_free, current_free->taille_total, 1, current_free->ptr_next_free);
      curseur += current_free->taille_total;
    }
    else
    {
      mem_busy_block_t *current_busy = ((mem_busy_block_t *)curseur);
      print(current_busy, current_busy->taille_total, 0, NULL);
