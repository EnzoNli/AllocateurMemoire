...
int main(int argc, char *argv[]) {
	...
	int free = 0;
	//On alloue en boucle des bloc de tailles variable et aléatoire comprise entre
	//1 et MAX_BLOC
	while ((i < MAX_ALLOC) && (allocs[i] = mem_alloc(size)) != NULL) {
		printf("%d -------------------------------\n", i);
		printf("Allocation en %d\n", (int)((char*)allocs[i]-(char*)get_memory_adr()));
		assert(allocs[i]<(void*)((char*)get_memory_adr()+get_memory_size()));
		...
		if (rand()%FREQ_FREE == 0) {
			free = ((rand()%(i+1))-1);
			printf("Libération %d\n", free);
			assert(allocs[free]<(void*)((char*)get_memory_adr()+get_memory_size()));
	...
	if (mem_alloc(size) == NULL) {
		...
		printf("Le tableau d'allocation est trop petit, augmentez MAX_ALLOC ou MAX_BLOC\n");
	}
